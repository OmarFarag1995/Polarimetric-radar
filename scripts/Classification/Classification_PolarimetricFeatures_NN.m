...
for i = 1:k
    ...
    % Train Acccuracy 
    Y_train_pred = classify(model, X_train);
    ...
    % Test Data Accuracy 
    % Make predictions
    Y_test_pred = classify(model, X_test);
...
% Save the best model
save('..\Saved_Models\PolarimetricFeatures_NNModel.mat', 'bestModel', 'mu', 'sigma');

% Evaluate the final model on the entire dataset (optional)
Y_pred_best = classify(bestModel, X);
