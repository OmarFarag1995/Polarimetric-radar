...
for iLook = 1:length(lookPaths)
    ...
    for iField = 1:length(refFields)
        ...
        % --- Normalize the sequence (z-score) ---
        mu = mean(channelData, 2);
        sigma = std(channelData, 0, 2);
        if sigma == 0
            sigma = 1; % Prevent division by zero for flat sequences
        end
        channelDataNorm = (channelData - mu) ./ sigma;

        allData{end+1} = channelDataNorm;
...
modelFilename = 'Data/Models/LSTM_RoadSurface_AllLOOKs_Normalized.mat';
